...
export default async function EditPagePage({
  ...
  return (
    <div>
      <h1 className="mb-2 font-['Instrument_Serif',serif] text-3xl capitalize">{key}</h1>
      <p className="mb-6 text-sm text-[#948d7e]">
        Edit the page content as JSON. Make sure it stays valid JSON before saving.
      </p>

      {error === "invalid-json" && (
        <p className="mb-4 rounded-md border border-red-400 bg-red-50 px-3 py-2 text-sm text-red-500">
          That wasn&apos;t valid JSON &mdash; nothing was saved.
        </p>
      )}
      ...
      <form action={savePageAction} className="flex flex-col gap-4">
        ...
        <textarea
          ...
          className="rounded-md border border-[#211f1a]/15 bg-white px-3 py-2 font-mono text-sm text-[#211f1a] outline-none focus:border-[#ba923a]"
        />
        <div>
          <button
            type="submit"
